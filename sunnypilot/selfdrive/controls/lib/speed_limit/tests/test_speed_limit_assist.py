...
class TestSpeedLimitAssist:
  ...
  def test_disabled(self):
    ...
    for _ in range(int(10. / DT_MDL)):
      self.sla.update(True, False, SPEED_LIMITS['city'], 0, SPEED_LIMITS['highway'], SPEED_LIMITS['city'], SPEED_LIMITS['city'],
                      True, 0, self.events_sp, self.sm['carState'])
    assert self.sla.state == SpeedLimitAssistState.disabled
  ...
  def test_preactive_to_active_with_max_speed_confirmation(self):
    ...
    self.sla.update(True, False, SPEED_LIMITS['city'], 0, self.pcm_long_max_set_speed, SPEED_LIMITS['city'], SPEED_LIMITS['city'],
                    True, 0, self.events_sp, self.sm['carState'])
  ...
  def test_preactive_to_pending_no_speed_limit(self):
    self.sla.state = SpeedLimitAssistState.preActive
    self.sla.update(True, False, SPEED_LIMITS['city'], 0, self.pcm_long_max_set_speed, 0, 0,
                    False, 0, self.events_sp, self.sm['carState'])
  ...
  def test_pending_to_adapting_when_below_speed_limit(self):
    self.sla.state = SpeedLimitAssistState.pending
    self.sla.update(True, False, SPEED_LIMITS['city'] + 5, 0, self.pcm_long_max_set_speed, SPEED_LIMITS['city'], SPEED_LIMITS['city'],
                    True, 0, self.events_sp, self.sm['carState'])
  ...
  def test_active_to_adapting_transition(self):
    ...
    self.sla.update(True, False, SPEED_LIMITS['city'] + 2, 0, self.pcm_long_max_set_speed, SPEED_LIMITS['city'], SPEED_LIMITS['city'],
                    True, 0, self.events_sp, self.sm['carState'])
    assert self.sla.state == SpeedLimitAssistState.adapting
  ...
  def test_manual_cruise_change_detection(self):
    ...
    self.sla.update(True, False, SPEED_LIMITS['city'], 0, different_cruise, SPEED_LIMITS['city'], SPEED_LIMITS['city'],
                    True, 0, self.events_sp, self.sm['carState'])
    assert self.sla.state == SpeedLimitAssistState.inactive
  ...
  def test_rapid_speed_limit_changes(self):
    ...
    for _, speed_limit in enumerate(speed_limits):
      self.sla.update(True, False, speed_limit, 0, self.pcm_long_max_set_speed, speed_limit, speed_limit,
                      True, 0, self.events_sp, self.sm['carState'])
  ...
  def test_invalid_speed_limits_handling(self):
    ...
    for invalid_limit in invalid_limits:
      self.sla.update(True, False, SPEED_LIMITS['city'], 0, self.pcm_long_max_set_speed, invalid_limit, SPEED_LIMITS['city'],
                      True, 0, self.events_sp, self.sm['carState'])
  ...
  def test_stale_data_handling(self):
    ...
    self.sla.update(True, False, SPEED_LIMITS['city'], 0, self.pcm_long_max_set_speed, 0, old_speed_limit,
                    True, 0, self.events_sp, self.sm['carState'])
    assert self.sla.state in ACTIVE_STATES
  ...
  def test_distance_based_adapting(self):
    ...
    self.sla.update(True, False, current_speed, 0, self.pcm_long_max_set_speed, target_speed, target_speed,
                    True, distance, self.events_sp, self.sm['carState'])
    assert self.sla.state == SpeedLimitAssistState.adapting

...
MAX_ACCEL_PROFILES = {
  ...
  AccelPersonality.normal:    [2.00, 2.0,  1.95, 1.55, .60,  .295, .18, .13],   # normal
  AccelPersonality.sport:     [2.00, 2.0,  1.99, 1.85, .70,  .333, .24, .2],   # sport
}
MAX_ACCEL_BREAKPOINTS =       [0.,   4.,   6.,   9.,   16.,  25., 30., 55.]

# Braking profiles mapped to LongPersonality (relaxed/standard/aggressive)
MIN_ACCEL_PROFILES = {
  LongPersonality.relaxed:    [-0.12, -1.0, -1.0],  # gentler braking
  LongPersonality.standard:   [-0.12, -1.1, -1.1],  # normal braking
  LongPersonality.aggressive: [-0.12, -1.2, -1.2],  # more aggressive braking
}
...
class VibePersonalityController:
    ...
    def _get_toggle_state(self, key: str) -> bool:
        return bool(self.params.get_bool(self.param_keys[key]))
    ...
    def is_enabled(self) -> bool: return (self._get_toggle_state('enabled')
                                          and (self._get_toggle_state('accel_enabled')
                                               or self._get_toggle_state('follow_enabled')))
    ...
    def get_follow_distance_multiplier(self, v_ego: float) -> float:
        ...
        self._update_from_params()
        return float(get_dynamic_personality(v_ego, self.long_personality))

...
class CarControllerParams:
  STEER_STEP = 1
  STEER_MAX = 1500  # Default class STEER_MAX
  ...
  def __init__(self, CP):
    ...
    if self.CP.carFingerprint == CAR.TOYOTA_PRIUS:
      self.STEER_MAX = 1875  # Instance STEER_MAX for Prius
    else:
      self.STEER_MAX = CarControllerParams.STEER_MAX # Use class variable for others

    if self.CP.carFingerprint == CAR.TOYOTA_PRIUS:
      if CP.lateralTuning.which() == 'torque':
        self.STEER_DELTA_UP = 19
        self.STEER_DELTA_DOWN = 32
      else:
        self.STEER_DELTA_UP = 13
        self.STEER_DELTA_DOWN = 32
    else: # Default values for other cars
      if CP.lateralTuning.which() == 'torque':

...
static int honda_rx_hook(CANPacket_t *to_push) {
  ...
  if (valid) {
    ...
    if (((addr == 0x1A6) || (addr == 0x296))) {
      ...
      if ((button2 == 1) && !lkas_pressed_prev) {
        mads_enabled = !mads_enabled;
        if (mads_enabled && ((alternative_experience & ALT_EXP_ENABLE_MADS) || (alternative_experience & ALT_EXP_MADS_DISABLE_DISENGAGE_LATERAL_ON_BRAKE))) {
          controls_allowed = 1;
        }
      }

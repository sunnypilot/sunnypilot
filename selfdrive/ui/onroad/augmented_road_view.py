...
class AlertRenderer(Widget):
  def __init__(self):
    ...
    self._anim_duration = 0.28  # nice smooth slide

  def _anim_values(self, alert_id: str):
    """
    Slide-down animation:
    Starts above screen (-120px) → final position.
    Cubic ease-out.
    """
    now = rl.get_time()
    ...
    y_offset = (1 - eased) * -120  # slide from -120px to 0px
    fade = eased                   # fade in with same curve
  ...
  def _render(self, rect: rl.Rectangle):
    ...
    # Compute base rectangle (TOP of screen instead of bottom)
    alert_rect = self._get_alert_rect(rect, alert.size)

    # Animate ONLY normal (non-critical) alerts
    if alert.status == AlertStatus.normal:
      alert_id = f"{alert.text1}-{alert.text2}-{alert.size}"
      fade, y_offset = self._anim_values(alert_id)

      animated_rect = rl.Rectangle(
        alert_rect.x,
        alert_rect.y + y_offset,  # slide from top
        alert_rect.width,
        alert_rect.height
      )
    else:
      animated_rect = alert_rect
      fade = 1.0

    # Draw background with fade applied
    bg_color = ALERT_COLORS.get(alert.status, ALERT_COLORS[AlertStatus.normal])
    bg_color = rl.Color(bg_color.r, bg_color.g, bg_color.b, int(bg_color.a * fade))

    if alert.size != AlertSize.full:
      roundness = ALERT_BORDER_RADIUS / (min(animated_rect.width, animated_rect.height) / 2)
      rl.draw_rectangle_rounded(animated_rect, roundness, 10, bg_color)
    else:
      rl.draw_rectangle_rec(animated_rect, bg_color)

    # Text rectangle
    text_rect = rl.Rectangle(
      animated_rect.x + ALERT_PADDING,
      animated_rect.y + ALERT_PADDING,
      animated_rect.width - 2 * ALERT_PADDING,
      animated_rect.height - 2 * ALERT_PADDING
    )

    # Draw text with fade
    self._draw_text(text_rect, alert, fade_override=fade)

  def _get_alert_rect(self, rect: rl.Rectangle, size: int) -> rl.Rectangle:
    """
    ⚠ MOVED ALERTS TO **TOP** OF SCREEN.
    """
  ...
  def _draw_text(self, rect: rl.Rectangle, alert: Alert, fade_override=1.0) -> None:
    color_white = rl.Color(255, 255, 255, int(255 * fade_override))

    if alert.size == AlertSize.small:
      self._draw_centered(alert.text1, rect, self.font_bold, ALERT_FONT_MEDIUM, color=color_white)

    elif alert.size == AlertSize.mid:
      self._draw_centered(alert.text1, rect, self.font_bold, ALERT_FONT_BIG, center_y=False, color=color_white)
      rect.y += ALERT_FONT_BIG + ALERT_LINE_SPACING
      self._draw_centered(alert.text2, rect, self.font_regular, ALERT_FONT_SMALL, center_y=False, color=color_white)
      ...
      self._full_text1_label._color = color_white
      ...
      self._full_text2_label._color = color_white

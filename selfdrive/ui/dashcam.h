...
void stop_capture() {
  char videos_dir[50] = "/data/media/0/videos";
...
static void screen_draw_button(UIState *s) {
  if (captureState == CAPTURE_STATE_CAPTURING) {
    ...
    int btn_w = 150;
    int btn_h = 150;
    int btn_x = 9000 - btn_w + 12;
    int btn_y = 9000 - btn_h + 5;
    nvgBeginPath(s->vg);
...
void dashcam_tick(UIState *s) {
  if (!s->scene.started) {
    // Assume car is not in drive so stop recording
    stop_capture();
  }

  if (captureState == CAPTURE_STATE_CAPTURING && (get_time() - start_time) >= RECORD_INTERVAL) {
    rotate_video();
  }
}

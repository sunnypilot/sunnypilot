...
class Colors:
  ...
  GOOD = rl.Color(210, 246, 230, 255)      # soft mint
  WARNING = rl.Color(252, 231, 169, 255)   # soft amber
  DANGER = rl.Color(250, 205, 205, 255)    # soft red

  # UI elements
  METRIC_BORDER = rl.Color(255, 255, 255, 75)
...
class Sidebar(Widget):
  def __init__(self):
    ...
    # Callbacks (kept compatible with main.py)
    self._on_settings_click: Callable | None = None
    self._on_flag_click: Callable | None = None
    self._open_settings_callback: Callable | None = None

  # IMPORTANT: signature must match main.py usage
  def set_callbacks(
    self,
    on_settings: Callable | None = None,
    on_flag: Callable | None = None,
    open_settings: Callable | None = None,
  ):
    """
    on_settings: called when the settings button is pressed
    on_flag:     called when the home/flag button is pressed while onroad
    open_settings: called when tapping the mic indicator (to jump to settings)
    """
  ...
  def _render(self, rect: rl.Rectangle):
    ...
    # Top logo area could be added here if you want a Konik/hoofpilot mark
  ...
  def _handle_mouse_release(self, mouse_pos: MousePos):
    ...
    # Home / flag (bookmark) button — only clickable when started
    if rl.check_collision_point_rec(mouse_pos, HOME_BTN) and ui_state.started:
      if self._on_flag_click:
        self._on_flag_click()
      return

    # Mic “recording” pill — opens settings
  ...
  def _draw_buttons(self, rect: rl.Rectangle):
    ...
    # Mic indicator pill (shows when recording_audio is True)
    if self._recording_audio:
      ...
      base = Colors.DANGER
      bg_color = rl.Color(base.r, base.g, base.b, int(255 * (0.65 if mic_pressed else 0.9)))
  ...
  def _draw_metrics(self, rect: rl.Rectangle):
    # Three vertical cards: TEMP, VEHICLE, STABLE (cloud)
    metrics = [
      (self._temp_status, 338),
      (self._panda_status, 496),
      (self._connect_status, 654),
    ]
  ...
  def _draw_metric(self, rect: rl.Rectangle, metric: MetricData, y: float):
    ...
    # Soft colored strip on the left

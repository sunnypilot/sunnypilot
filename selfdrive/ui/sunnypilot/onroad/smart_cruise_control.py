...
class SmartCruiseControlRenderer(Widget):
  ...
  def _draw_icon(self, rect_center_x, rect_height, x_offset, y_offset, name):
    ...
    rl.draw_rectangle_rounded(bg_rect, 0.2, 10, box_color)

    text_pos_x = bg_rect.x + (bg_rect.width - sz.x) / 2
    text_pos_y = bg_rect.y + (bg_rect.height - sz.y) / 2

    # Using Black text nicely simulates "subtracted" text given high contrast with Green/Grey

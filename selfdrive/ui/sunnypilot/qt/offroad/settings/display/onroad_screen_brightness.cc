...
OnroadScreenBrightnessControl::OnroadScreenBrightnessControl(const QString &param, const QString &title, const QString &description, const QString &icon, QWidget *parent)
 ...
 onroadScreenOffTimer = new  OptionControlSP(
  ...
  "",
  "",
  {-1, 11}, 1, true, &onroadScreenOffTimerOptions);
...
void OnroadScreenBrightnessControl::refresh() {
 ...
 labelTimer += (valTimer == -1 ? " Always On" : valTimer < 60 ? std::to_string(valTimer) + "s" : std::to_string(valTimer / 60) + "m");
 ...
 if (valTimer == -1) {
  onroadScreenBrightness->setEnabled(false);
  valBrightness = "100";
 } else {
  onroadScreenBrightness->setEnabled(true);
 }
 ...
 labelBrightness += (valBrightness == "0" ? " Dark" : valBrightness + "%");

...
class ModelsLayout(Widget):
  ...
  def _handle_current_model_clicked(self):
    ...
    for folder, bundles in sorted(folders.items(), key=lambda x: max((bundle.index for bundle in x[1]), default=-1), reverse=True):
      ...
      folders_list.append(TreeFolder(name, [TreeNode(folder, bundle.displayName, bundle.ref, bundle.index) for bundle in bundles]))

    if (favs := self._params.get("ModelManager_Favs")) and (fav_bundles := [bundle for bundle in bundles if bundle.ref in set(favs.split(';'))]):
      folders_list.insert(1, TreeFolder("Favorites", [TreeNode("", bundle.displayName, bundle.ref, bundle.index) for bundle in fav_bundles]))
  ...
  def _show_reset_params_dialog(self):
    ...
    msg = tr("Model download has started in the background. We suggest you to reset calibration. Would you like to do that now?")

...
class CarState(CarStateBase):
  ...
  def update(self, cp, cp_cam):
    ...
    if not self.CP.openpilotLongitudinalControl and self.CP.carFingerprint not in NON_SCC_NO_FCA_CAR:
      aeb_src = "FCA11" if self.CP.flags & HyundaiFlags.USE_FCA else "SCC12"
      aeb_sig = "FCA_CmdAct" if self.CP.flags & HyundaiFlags.USE_FCA else "AEB_CmdAct"
      ...
      aeb_sig = "FCA_CmdAct" if self.CP.flags & HyundaiFlags.USE_FCA else "AEB_CmdAct"
      aeb_warning_sig = "CF_VSM_Warn" if self.CP.flags & HyundaiFlags.USE_FCA else "CF_VSM_Warn_SCC12"
      aeb_braking_sig = "CF_VSM_DecCmdAct" if self.CP.flags & HyundaiFlags.USE_FCA else "CF_VSM_DecCmdAct_SCC12"
      aeb_braking_cmd = "CR_VSM_DecCmd_FCA11" if self.CP.flags & HyundaiFlags.USE_FCA else "CR_VSM_DecCmd_SCC12"

...
def create_lkas11(packer, frame, car_fingerprint, apply_steer, steer_req,
                  lkas11, sys_warning, sys_state, enabled, disengage_from_brakes,
  ...
  if car_fingerprint in [CAR.SONATA, CAR.PALISADE, CAR.KIA_NIRO_EV, CAR.KIA_NIRO_HEV_2021, CAR.SANTA_FE,
    ...
    values["CF_Lkas_FcwOpt_USM"] = 2 if enabled else 1 if disengage_from_brakes else 1
...
def create_lfahda_mfc(packer, enabled, disengage_from_brakes, hda_set_speed=0):
  values = {
    "LFA_Icon_State": 2 if enabled else 1 if disengage_from_brakes else 0,

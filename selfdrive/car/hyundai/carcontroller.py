  ...
  def update(self, CC, CS, now_nanos):
    ...
    if self.CP.carFingerprint in CANFD_CAR or (hda2 and can_canfd_hybrid):
      ...
      if self.frame % 5 == 0 and (not hda2 or hda2_long):
        can_sends.append(hyundaicanfd.create_lfahda_cluster(self.packer, self.CAN, CC.enabled and CS.out.cruiseState.enabled, can_canfd_hybrid, CC.latActive,
                                                            lateral_paused, blinking_icon))
      ...
      if self.CP.openpilotLongitudinalControl:
        ...
        if self.frame % 2 == 0:
          if can_canfd_hybrid:
            ...
            can_sends.extend(hyundaicanfd.create_acc_commands_can_canfd_hybrid(self.packer, self.CAN, CC.enabled and CS.out.cruiseState.enabled, accel, self.accel_last, upper_jerk,
                                                                               int(self.frame / 2), hud_control.leadVisible,
                                                                               set_speed_in_units, stopping, CC.cruiseControl.override, CS.gac_tr_cluster, CS.mainEnabled))
          else:

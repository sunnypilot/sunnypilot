...
class CarController:
  ...
  def update(self, CC, CS):
    ...
    if self.frame % 2 == 0 and self.CP.openpilotLongitudinalControl:
      ...
      can_sends.extend(create_acc_commands(self.packer, CC.enabled and CS.out.cruiseState.enabled and CS.mainEnabled, accel, jerk, int(self.frame / 2), lead_visible,
                                           set_speed_in_units, stopping, CS.out.gasPressed, CS.mainEnabled))
      self.accel = accel
    ...
    if self.frame % 5 == 0 and self.car_fingerprint in (CAR.SONATA, CAR.PALISADE, CAR.IONIQ, CAR.KIA_NIRO_EV, CAR.KIA_NIRO_HEV_2021,
      ...
      can_sends.append(create_lfahda_mfc(self.packer, CC.latActive, disengage_from_brakes, below_lane_change_speed, disengage_blinking_icon))

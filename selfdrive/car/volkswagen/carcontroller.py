...
class CarController():
  ...
  def update(self, enabled, CS, frame, ext_bus, actuators, visual_alert, left_lane_visible, right_lane_visible, left_lane_depart, right_lane_depart):
    ...
    if CS.CP.pcmCruise:
      ...
      if frame > self.graMsgStartFramePrev + P.GRA_VBP_STEP * 0.25:
        if not ((not enabled and CS.out.cruiseState.enabled) and (enabled and CS.esp_hold_confirmation)) and (enabled and CS.cruise_active):

...
class CarController:
  ...
  def update(self, CC, CS, now_nanos):
    ...
    lkas_active = CC.latActive and CS.out.madsEnabled

    if self.frame % 10 == 0 and self.CP.carFingerprint not in RAM_CARS:
      can_sends.append(chryslercan.create_lkas_heartbit(self.packer, CS.out.madsEnabled, CS.lkas_heartbit))
    ...
    if self.frame % 25 == 0:
      if CS.lkas_car_model != -1:
        can_sends.append(chryslercan.create_lkas_hud(self.packer, self.CP, lkas_active, CS.out.madsEnabled, CC.hudControl.visualAlert,

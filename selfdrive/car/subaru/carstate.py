...
class CarState(CarStateBase):
  ...
  def update(self, cp, cp_cam, cp_body):
    ...
    if self.car_fingerprint in PREGLOBAL_CARS:
      self.acc_main_enabled = cp.vl["CruiseControl"]["Cruise_On"]
    else:
      self.acc_main_enabled = cp_cam.vl["ES_LKAS_State"]["LKAS_Dash_State"]

    if self.car_fingerprint not in PREGLOBAL_CARS or not CAR.CROSSTREK_2020H:
    ...
    if ret.cruiseState.available:
      if self.car_fingerprint in PREGLOBAL_CARS:
        if self.prev_cruise_button == 2: # SET_SHALLOW
          ...
          if self.cruise_button != 5 and self.resumeAvailable == True:
            self.accEnabled = True
      elif self.car_fingerprint not in PREGLOBAL_CARS or not CAR.CROSSTREK_2020H:
      ...
      if not self.disable_mads:
        if not self.prev_acc_main_enabled and self.acc_main_enabled:
          self.accMainEnabled = True
        elif self.prev_acc_main_enabled and not self.acc_main_enabled:
          self.accMainEnabled = False

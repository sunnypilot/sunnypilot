...
class ModelState(ModelState20Hz, ModelSwitcher, ModelSmartInput):
  ...
  def run(self, buf: VisionBuf, wbuf: VisionBuf, transform: np.ndarray, transform_wide: np.ndarray,
    ...
    if not self.is_20hz:
      ...
      self.numpy_inputs['features_buffer'][:] = self.get_full_features_20hz(outputs, self.full_features_20Hz_idxs)

    if not self.using_smart_input:
      ...
      self.process_outputs_smart(outputs)
    return outputs

...
class Controls:
  def __init__(self, sm=None, pm=None, can_sock=None, CI=None):
    ...
    if self.enable_mads:
      if self.mads_disengage_lateral_on_brake:
  ...
  def update_events(self, CS):
    ...
    if (CS.gasPressed and not self.CS_prev.gasPressed and self.disengage_on_accelerator) or \
      ...
      if CS.cruiseState.enabled:
        if not self.enable_mads or (self.enable_mads and self.mads_disengage_lateral_on_brake):
          self.events.add(EventName.pedalPressed)
      elif self.enable_mads and \
        (self.mads_disengage_lateral_on_brake or (not self.mads_disengage_lateral_on_brake and CS.standstill)):
        self.events.add(EventName.silentPedalPressed)

    extra_gears = [car.CarState.GearShifter.sport, car.CarState.GearShifter.low]
    if (CS.gearShifter != GearShifter.drive and CS.gearShifter not in extra_gears and
      not (CS.gearShifter == GearShifter.unknown and self.gear_warning < int(0.5/DT_CTRL)) and
      (self.enable_mads and not self.mads_disengage_lateral_on_brake)):
      self.events.add(EventName.silentPedalPressed)

    self.gear_warning = self.gear_warning + 1 if CS.gearShifter == GearShifter.unknown else 0
  ...
  def state_control(self, CS):
    ...
    extra_gears = [car.CarState.GearShifter.sport, car.CarState.GearShifter.low]
    wrong_gear = True if (CS.gearShifter != GearShifter.drive and CS.gearShifter not in extra_gears and
      not (CS.gearShifter == GearShifter.unknown and self.gear_warning < int(0.5/DT_CTRL)) and
      (self.enable_mads and not self.mads_disengage_lateral_on_brake)) else False

    self.gear_warning = self.gear_warning + 1 if CS.gearShifter == GearShifter.unknown else 0

    # Check which actuators can be enabled
    CC.latActive = (self.active or (self.enable_mads and not self.mads_disengage_lateral_on_brake)) and not CS.steerFaultTemporary and not CS.steerFaultPermanent and \
                   (CS.vEgo > self.CP.minSteerSpeed and not CS.standstill and (not self.enable_mads or (self.enable_mads and self.mads_disengage_lateral_on_brake) or (self.enable_mads and not self.mads_disengage_lateral_on_brake))) and CS.madsEnabled and \
                   (not CS.belowLaneChangeSpeed or (not (((self.sm.frame - self.last_blinker_frame) * DT_CTRL) < 1.0))) and not wrong_gear
    CC.longActive = self.active and CS.cruiseState.enabled and not self.events.any(ET.OVERRIDE) and self.CP.openpilotLongitudinalControl
  ...
  def publish_logs(self, CS, start_time, CC, lac_log):
    ...
    controlsState.madsEnabled = CS.madsEnabled
    controlsState.cruiseEnabled = CS.cruiseState.enabled
    controlsState.suspended = (CS.brakePressed and CS.disengageByBrake and self.mads_disengage_lateral_on_brake) or \
                               (self.enabled and CS.madsEnabled and not
                               ((self.active or (self.enable_mads and not self.mads_disengage_lateral_on_brake)) and not CS.steerFaultTemporary and not CS.steerFaultPermanent and
                               (CS.vEgo > self.CP.minSteerSpeed and not CS.standstill and (not self.enable_mads or (self.enable_mads and self.mads_disengage_lateral_on_brake) or (self.enable_mads and not self.mads_disengage_lateral_on_brake))) and CS.madsEnabled and

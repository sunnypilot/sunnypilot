...
class LateralPlanner:
  def __init__(self, CP):
    ...
    self.steering_rate_cost = 0
    self.param_s = Params()
    self._steering_rate_cost = 0
    self.frame = 0
    self.custom_src = False
    self.custom_src_timer = 0
  ...
  def live_tune(self):
    self.frame += 1
    if self.frame % 300 == 0:
      self._steering_rate_cost = int(self.param_s.get("CustomSteerRateCost", encoding="utf8"))
      self.steering_rate_cost = self._steering_rate_cost
      self.frame = 0

  def update(self, sm):
    self.custom_src_timer += 1
    if self.custom_src_timer > 100:
      self.custom_src_timer = 0
      self.custom_src = self.param_s.get_bool("CustomSteerRateCost")
    if self.custom_src:
      self.live_tune()
    else:
      self.steering_rate_cost = 800.0

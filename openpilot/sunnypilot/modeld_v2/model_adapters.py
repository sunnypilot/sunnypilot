...
from openpilot.sunnypilot.modeld_v2.helpers import DynamicTinygradUnpickler, patch_tinygrad_engine
...
class LegacyModelAdapter(BaseModelAdapter):
  def __init__(self, *args, **kwargs):
    ...
    if self.warp is not None:
      ...
      patch_tinygrad_engine()

      class CallArg:
        aux = "LEGACY_EXEC"

      def patch_linear(linear: UOp) -> UOp:
        if getattr(linear, "_patched", False):
          return linear
        new_src = []
        for call in linear.src:
          if call.op in (Ops.PROGRAM, Ops.COPY):
            new_ast = call.replace(src=())
            new_src.append(UOp(Ops.CALL, src=(new_ast, *call.src), arg=CallArg()))
          else:
            new_src.append(call)
        new_lin = linear.replace(src=tuple(new_src))
        new_lin._patched = True
        return new_lin

      for jit_obj in (self.warp, self.run_policy, self.run_model):
        if hasattr(jit_obj, 'captured') and jit_obj.captured is not None:
          jit_obj.captured.expected_input_info = AnyEq()
          jit_obj.captured.expected_names = AnyEq()
          if hasattr(jit_obj.captured, 'linear'):
            jit_obj.captured.linear = patch_linear(jit_obj.captured.linear)
          elif hasattr(jit_obj.captured, '_linear'):
            jit_obj.captured._linear = patch_linear(jit_obj.captured._linear)

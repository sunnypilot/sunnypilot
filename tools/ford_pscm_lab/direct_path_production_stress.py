"""Independent arithmetic, direct-command and CAN stress for v9 C0/C1 PI."""
import argparse
import hashlib
import json
import math
...
def stress(cycles, output):
  ...
  for i in range(cycles):
    ...
    if active:
      ...
      assert abs(core.c0-target_c0) <= 1e-10
      ...
      assert abs(core.c1-request) <= 1e-10
  ...
  result = {'cycles': cycles, 'kp': kp, 'ki': ki, 'controller_updates': 3*cycles,
            ...
            'checks': 'Independent scalar PI/unwind-first/anti-windup arithmetic, symmetry, C0 independence, bounds, direct commands, resets, CAN.',
